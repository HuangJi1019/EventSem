...
class Transformer(nn.Module):
    ...
    def forward(self, src, mask, pos_embed, video_length=None, saliency_proj1=None, saliency_proj2=None):
        ...
        #t2v_src 是经过 ACA 融合后的特征,attn_weights 是 Cross Attention 计算的注意力权重，表示每个视频片段和文本片段之间的相关性
        ...
        #显著性得分 (Saliency Score) 计算,表示每个视频片段的重要程度,这种显著性得分用于帮助模型找到与查询最匹配的视频时刻
        vid_mem = vid_fuse.permute(1, 0, 2).contiguous()  # 使用 permute+contiguous 代替 transpose
        memory_global = torch.mean(vid_mem, dim=1)
...
class TransformerCATEEncoder(nn.Module):
    def __init__(self, encoder_layer, num_layers, norm=None, return_intermediate=False):
        super().__init__()
        self.layers = _get_clones(encoder_layer, num_layers)# 克隆 Transformer Encoder 层
        self.num_layers = num_layers
        self.norm = norm
    ...
    def forward(self, src,
        ...
        intermediate = [] # 存储每一层的中间输出（仅在 `return_intermediate=True` 时使用）
        attn_weights = None# 累加每一层的注意力权重
        for i, layer in enumerate(self.layers):
        ...
        attn_weights /= self.num_layers #平均化注意力权重
