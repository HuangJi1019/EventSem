...
from EventSem.span_utils import span_xx_to_cxw
...
class StartEndDataset(Dataset):
    ...
    def __init__(self, dset_name, data_path, v_feat_dirs, q_feat_dir,
                 ...
                 dset_domain=None,semantic_t_feat_dir=None):
        ...
        self.semantic_t_feat_dir = semantic_t_feat_dir
        if max_v_l == -1:
            max_v_l = 100000000
        if max_q_l == -1:
            max_q_l = 100
    ...
    def _load_model_inputs(self, meta):
        ...
        if self.semantic_t_feat_dir:
            model_inputs["semantic_t_feat"]  = self._get_semantic_query_feat_by_qid(meta["qid"])
        return model_inputs
    ...
    def get_query(self, query):
        ...
        # print(f"分割后的 tokens: {tokens}")
        word_inds = torch.LongTensor(
            [self.vocab.stoi.get(w.lower(), 400000) for w in tokens])
        
        return self.embedding(word_inds)
    
    def basic_tokenize(self,query):
    ...
    def _get_semantic_query_feat_by_qid(self,qid):
        q_feat_path = join(self.semantic_t_feat_dir, f"{qid}.npy")
        q_feat = np.load(q_feat_path).astype(np.float32)
        if self.max_q_l != -1:
            q_feat = q_feat[:self.max_q_l]
        
        if self.dset_name == 'charadesSTA':
            current_q_l = q_feat.shape[0]
            if current_q_l > self.max_q_l:
                q_feat = q_feat[:self.max_q_l]
                current_q_l = self.max_q_l 
            if current_q_l < self.max_q_l:
                padding_length = self.max_q_l - current_q_l
                feature_dimension = q_feat.shape[1] if q_feat.ndim > 1 else 0
                padding = np.zeros((padding_length, feature_dimension), dtype=np.float32)
                q_feat = np.concatenate((q_feat, padding), axis=0)
        if self.normalize_t:
            q_feat = l2_normalize_np_array(q_feat)
        if self.txt_drop_ratio > 0:
            q_feat = self.random_drop_rows(q_feat)
...
def prepare_batch_inputs(batched_model_inputs, device='cuda', non_blocking=False):
    ...
    model_inputs = dict(
        ...
        semantic_t_feat = batched_model_inputs["semantic_t_feat"][0].to(device, non_blocking=non_blocking),
        semantic_t_feat_mask = batched_model_inputs["semantic_t_feat"][1].to(device, non_blocking=non_blocking) if "semantic_t_feat" in batched_model_inputs else None,
