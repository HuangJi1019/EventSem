...
from EventSem.config import TestOptions
from EventSem.start_end_dataset import (
    StartEndDataset,
    start_end_collate,
    prepare_batch_inputs,
)
from EventSem.postprocessing import PostProcessorDETR
...
def setup_model(opt):
    ...
    optimizer = torch.optim.AdamW(param_dicts, weight_decay=opt.wd)
    lr_scheduler = MultiStepLR(optimizer, milestones=opt.lr_drop, gamma=0.5)
...
def start_inference(train_opt=None, split=None, splitfile=None):
    ...
    eval_dataset = StartEndDataset(
        ...
        semantic_t_feat_dir = opt.semantic_t_feat_dir,
    )
    ...
    print(metrics_no_nms,metrics_nms)
    if opt.eval_split_name == "val":
        logger.info(
